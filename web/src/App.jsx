...
function Timeline({ tasks, catMap, doneMap, onToggle, onEdit, isToday, nowMin }) {
  ...
  return (
    <div className="relative" style={{height:(endH-startH)*60*PPM+20}}>
      {/* Hour rails */}
      {hours.map(h=>(
        <div key={h} className="absolute left-0 right-0 flex items-start" style={{top:(h-startH)*60*PPM}}>
          <div className="w-16 text-[10px] uppercase tracking-[0.2em] pt-1 select-none" style={mono("#6b5a45")}>
            {minToLabel(h*60)}
          </div>
          <div className="flex-1 border-t" style={{borderColor:"#1a141012"}}/>
        </div>
      ))}

      {/* Now line */}
      {isToday && nowMin >= startH*60 && nowMin <= endH*60 && (
        <div className="absolute left-14 right-0 z-10 pointer-events-none" style={{top:top(nowMin)}}>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full flex-shrink-0" style={{background:"#c2410c"}}/>
            <div className="flex-1 h-[1px]" style={{background:"#c2410c"}}/>
            <span className="text-[9px] uppercase tracking-widest px-1.5 py-0.5 text-white flex-shrink-0"
      ...
      <div className="absolute left-16 right-0 top-0 bottom-0">
        {tasks.map(t=>{
          ...
          const height = Math.max(38, t.duration*PPM-4);
          const isDone = !!doneMap[t.id];
          return (
            <div key={t.id} className="absolute left-0 right-0" style={{top:top(toMin(t.start)),height}}>
              <div onClick={()=>onEdit(t)} className="h-full ml-2 pl-4 pr-3 py-2 cursor-pointer transition hover:translate-x-0.5 overflow-hidden"
                style={{
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  {height<=56 && <span className="text-[10px] flex-shrink-0 pt-0.5" style={mono("#6b5a45")}>{t.duration}m</span>}
                </div>
              </div>
            </div>
          );
