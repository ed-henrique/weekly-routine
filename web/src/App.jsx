...
function Timeline({ tasks, catMap, doneMap, onToggle, onEdit, isToday, nowMin }) {
  ...
  return (
    <div className="relative select-none" style={{ height: px((endH - startH) * 60) + 32 }}>

      {/* Hour rails — full-width rule + label, both anchored to the same px() value */}
      {hours.map(h => {
        const y = px((h - startH) * 60);
        return (
          <div key={h} className="absolute left-0 right-0" style={{ top: y }}>
            {/* Rule spans full width */}
            <div className="absolute left-0 right-0 border-t" style={{ borderColor: "#1a141012" }} />
            {/* Label sits just below the rule, no padding offset */}
            <div className="absolute left-0 w-14 text-[10px] uppercase tracking-[0.2em]"
              style={{ top: 3, ...mono("#6b5a45") }}>
              {minToLabel(h * 60)}
            </div>
          </div>
        );
      ...
      {/* Now line — uses same px() so it snaps to the same grid */}
      {isToday && nowMin >= startH * 60 && nowMin <= endH * 60 && (
        <div className="absolute left-0 right-0 z-10 pointer-events-none" style={{ top: px(nowMin - startH * 60) }}>
          <div className="absolute left-0 right-0 flex items-center">
            <div className="w-14 flex justify-end pr-1.5">
              <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: "#c2410c" }} />
            </div>
            <div className="flex-1 h-[1px]" style={{ background: "#c2410c" }} />            <span className="text-[9px] uppercase tracking-widest px-1.5 py-0.5 text-white flex-shrink-0"
              style={{ background: "#c2410c", ...mono() }}>
              now · {minToLabel(nowMin)}
            </span>
          </div>
        </div>
      )}

      {/* Task blocks — positioned in the same coordinate space as the rails */}
      {tasks.map(t => {
        const cat = catMap[t.category] || { color: "#475569", name: "—" };
        const topPx = px(toMin(t.start) - startH * 60);
        const fullH = Math.max(BLOCK_GAP + 1, Math.round(t.duration * PPM));
        const innerH = fullH - BLOCK_GAP; // visible block height, gap is transparent space below
        const isDone = !!doneMap[t.id];
        return (
          <div key={t.id} className="absolute" style={{ top: topPx, left: 56, right: 0, height: fullH }}>
            <div onClick={() => onEdit(t)}
              className="absolute left-2 right-0 cursor-pointer transition-transform hover:translate-x-0.5 overflow-hidden"
              style={{
                top: 0,
                height: innerH,
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={e => { e.stopPropagation(); onToggle(t.id); }}
                      ...
                      {isDone
                        ...
                        : <Circle size={18} style={{ color: "#1a141030" }} />}
                    </button>
                    <h3 className={`font-semibold text-[15px] leading-tight truncate ${isDone ? "line-through opacity-50" : ""}`}
                      style={{ fontVariationSettings: "'opsz' 36" }}>
                      {t.title}
                    </h3>
                  </div>
                  {innerH > 56 && (
                    <div className="mt-1 ml-7 flex items-center gap-3 text-[11px]" style={mono("#6b5a45")}>
                      ...
                      {t.repeat !== "none" && t.source === "routine" &&
                        <span className="flex items-center gap-1"><Repeat size={10} />{t.repeat}</span>}
                    </div>
                  )}
                </div>
                {innerH <= 56 && (
                  <span className="text-[10px] flex-shrink-0 pt-0.5" style={mono("#6b5a45")}>{t.duration}m</span>
                )}
              </div>
            </div>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-md" onClick={e => e.stopPropagation()}
        style={{
          background: "#faf5e9", fontFamily: "'Fraunces',Georgia,serif",
